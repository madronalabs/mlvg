...
NativeDrawBuffer* toNativeBuffer(void* p) { return static_cast<NativeDrawBuffer*>(p); }
NativeDrawBuffer* getNativeBuffer(ml::Layer* pLayer) { return static_cast<NativeDrawBuffer*>(pLayer->_buf); }


VectorImage* getVectorImage(const DrawContext& dc, Path imageName)
{
  const VectorImageTree& t = *(dc.pVectorImages);
  auto& img (t[imageName]);
  if(img)
  {
    return img.get();
  }
...
ml::Layer::~Layer()
{
  ...
  nvgDeleteFramebuffer(toNativeBuffer(_buf));
}
...
void drawToLayer(const Layer* pLayer)
{
  if(pLayer)
  {
    nvgBindFramebuffer(toNativeBuffer(pLayer->_buf));
  }
  else
  {
